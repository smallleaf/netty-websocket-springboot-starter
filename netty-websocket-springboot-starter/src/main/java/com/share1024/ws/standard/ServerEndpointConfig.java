package com.share1024.ws.standard;
...
public class ServerEndpointConfig {
    ...
    public ServerEndpointConfig(ServerEndpointConfigProperties endpointConfigProperties){
        if (StringUtils.isEmpty(endpointConfigProperties.getHost()) || "0.0.0.0".equals(endpointConfigProperties.getHost()) || "0.0.0.0/0.0.0.0".equals(endpointConfigProperties.getHost())) {
            this.HOST = "0.0.0.0";
        } else {
            this.HOST = endpointConfigProperties.getHost();
        }

        this.PORT = getAvailablePort(endpointConfigProperties.getPort());
        this.BOSS_LOOP_GROUP_THREADS = endpointConfigProperties.getBossLoopGroupThreads();
        this.WORKER_LOOP_GROUP_THREADS = endpointConfigProperties.getWorkerLoopGroupThreads();
        this.USE_COMPRESSION_HANDLER = endpointConfigProperties.isUseCompressionHandler();
        this.CONNECT_TIMEOUT_MILLIS = endpointConfigProperties.getOptionConnectTimeoutMillis();
        this.SO_BACKLOG = endpointConfigProperties.getOptionSoBacklog();
        this.WRITE_SPIN_COUNT = endpointConfigProperties.getChildOptionWriteSpinCount();
        this.WRITE_BUFFER_HIGH_WATER_MARK = endpointConfigProperties.getChildOptionWriteBufferHighWaterMark();
        this.WRITE_BUFFER_LOW_WATER_MARK = endpointConfigProperties.getChildOptionWriteBufferLowWaterMark();
        this.SO_RCVBUF = endpointConfigProperties.getChildOptionSoRcvbuf();
        this.SO_SNDBUF = endpointConfigProperties.getChildOptionSoSndbuf();
        this.TCP_NODELAY = endpointConfigProperties.isChildOptionTcpNodelay();
        this.SO_KEEPALIVE = endpointConfigProperties.isChildOptionSoKeepalive();
        this.SO_LINGER = endpointConfigProperties.getChildOptionSoLinger();
        this.ALLOW_HALF_CLOSURE = endpointConfigProperties.isChildOptionAllowHalfClosure();
        this.READER_IDLE_TIME_SECONDS = endpointConfigProperties.getReaderIdleTimeSeconds();
        this.WRITER_IDLE_TIME_SECONDS = endpointConfigProperties.getWriterIdleTimeSeconds();
        this.ALL_IDLE_TIME_SECONDS = endpointConfigProperties.getAllIdleTimeSeconds();
        this.MAX_FRAME_PAYLOAD_LENGTH = endpointConfigProperties.getMaxFramePayloadLength();
        this.USE_EVENT_EXECUTOR_GROUP = endpointConfigProperties.isUseEventExecutorGroup();
        this.EVENT_EXECUTOR_GROUP_THREADS = endpointConfigProperties.getEventExecutorGroupThreads();

        this.KEY_PASSWORD = endpointConfigProperties.getSslKeyPassword();
        this.KEY_STORE = endpointConfigProperties.getSslKeyStore();
        this.KEY_STORE_PASSWORD = endpointConfigProperties.getSslKeyStorePassword();
        this.KEY_STORE_TYPE = endpointConfigProperties.getSslTrustStoreType();
        this.TRUST_STORE = endpointConfigProperties.getSslTrustStore();
        this.TRUST_STORE_PASSWORD = endpointConfigProperties.getSslTrustStorePassword();
        this.TRUST_STORE_TYPE = endpointConfigProperties.getSslTrustStoreType();

        this.CORS_ORIGINS = endpointConfigProperties.getCorsOrigins();
        this.CORS_ALLOW_CREDENTIALS = endpointConfigProperties.isCorsAllowCredentials();
    }
